...
try: #main
  ...
  for sample in part1:
    ...
    for f in opcodes[op]:
      ...
      if f(*args)!=after:
        opcodes[op].remove(f)
  while (len(codeops)<len(opcodes)):
    for op in range(len(opcodes)):
      if len(opcodes[op])==1:
        found=opcodes[op][0]
        for op2 in opcodes:
          if found in opcodes[op2]:
            opcodes[op2].remove(found)
        codeops[op]=found
  #Part2
  realRegs=[0,0,0,0]
  for instructions in part2:
    op=instructions[0]
    args=instructions[1:]+[realRegs]
    codeops[op](*args)
  print "Final registers are:",realRegs
  print "Final answer is:",realRegs[0]
except Exception as e:
