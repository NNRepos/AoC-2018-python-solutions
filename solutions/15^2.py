...
try:
  ...
  while binLow<binHigh:
    ...
    for y in range(Ysize):
      for x in range(Xsize):
        if cave [y][x]=='E':
          ...
          units.append([unitX, unitY, unitRace, elfDMG, unitHP])
          elfCount+=1
        elif cave[y][x]=='G':
          unitX,unitY=x,y
          unitRace=cave[y][x]
    ...
    if WinningRace=='E':
      ...
      race='Aliens'
    if len(units)<elfCount:
      binLow=elfDMG+1
    else:
      binHigh=elfDMG
      finalAns=ans
    print "Results when elves have",elfDMG, "attack power:"
    print "The",race,"have won!\nAfter",round,"rounds, there are",\
    str(len(units))+'/'+str(elfCount),race,"left,"
    print "the sum of their hp is",HPSum,", and the final answer is:",ans
    print
  print "...And the final answer is:",finalAns
except Exception as e:
  print str(e),sys.exc_info()[2].tb_lineno
finally:
  raw_input()