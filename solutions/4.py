...
try:
  ...
  logs=[]
  for i in input:
    i=re.findall(r'[\d\w]+',i)[1:]
    logs.append(i)
  sortedLog=sorted(logs,key=op.itemgetter(0,1,2,3))
  guards=dict();
  currGuard=0;
  for log in sortedLog:
    minutes, one, two = log[3:6];
    minutes = int(minutes);
    if one == 'Guard':
      currGuard=two;
      if not two in guards:
        guards[two]=0;
    elif one == 'falls':
      guards[currGuard]-=minutes;
    elif one == 'wakes':
      guards[currGuard]+=minutes;
    else:
    ...
    # print log
    # raw_input()
  maxGuard=sortedLog[0][5]
  for guard in guards:
    if guards[guard] > guards[maxGuard]:
      maxGuard=guard
  guardMinutes=[0]*60;
  curr=False;
  startMin=0;
  for log in sortedLog:
    ...
    if one == 'Guard':
      ...
      raise Exception('bad one');
  maxMinute=0;
  for i,j in enumerate(guardMinutes):
    if j > guardMinutes[maxMinute]:
      maxMinute=i;
  print "ans = guard * minute =",maxGuard,"*",maxMinute,"=",int(maxGuard) * maxMinute
