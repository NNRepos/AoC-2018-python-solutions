...
try:
  ...
  for i in range(len(input)):
    input[i]=re.findall(r'-?\d+',input[i])
    input[i]=map(int,input[i])
  print "Maximize the terminal, wait until the first output appears, then hit enter (at least 8 times) to watch them collide"
  #tick
  for k in range(20000):
    ...
    for i in range(len(input)-1):
      if (abs(input[i][0]-input[i+1][0]) + abs(input[i][1]-input[i+1][1])) > 200:
        flag = 0
    if flag:
      #find range
      minX=maxX=input[0][0]
      ...
      extraX=min(0,minX)
      extraY=min(0,minY)
      print "coordinates (xXyY):",minX,maxX,minY,maxY
      for i in input:
        y=i[1]-extraY
        x=i[0]-extraX
        grid[y][x]='#'
      for row in grid[minY-extraY-1:maxY-extraY+1]:
        print ''.join(row[minX-extraX-1:maxX-extraX+1]) #NBHEZHCJ
      raw_input()
    #move items
    for i in range(len(input)):
      input[i][0]+=input[i][2]
