...
try:
  ...
  min_boost=0
  max_boost=1000
  while (min_boost<=max_boost): #binary search
    curr_boost=(min_boost+max_boost)/2
    splinput=input.split('\n\n')
    immune=splinput[0].split('\n')[1:]
    infection=splinput[1].split('\n')[1:]
    immune_groups=[]
    infection_groups=[]
    ...
    for group in immune:
      ...
      # dmg=int(match.group('dmg')) #part 1
      dmg=int(match.group('dmg'))+curr_boost #part 2
    ...
    while (8): #fight
      ...
      #sort both groups by effective power and init
      ...
      if over: #all immune groups are dead
        for group in infection_groups: #count living units
          if not group[7]:
            ans+=group[0]
        print "boost", str(curr_boost) + ": The infection has won after", rounds, "rounds, with", ans, "units remaining."
        winner="infection"
        break;
        ...
        if over: #all infection groups are dead
          for group in immune_groups: #count living units
            if not group[7]:
              ans+=group[0]
          print "boost", str(curr_boost) + ": The immune system has won after", rounds, "rounds, with", ans, "units remaining."
          winner="immune"
          break;
      #war not over
      rounds+=1
      if rounds>10000: #draw
        print "boost", str(curr_boost)+": Reached round 10000, looks like we have a draw."
        winner="infection"
        break;
    if winner=="immune":
      max_boost=curr_boost-1
      last_units=ans
    else:
      min_boost=curr_boost+1
  print "the final answer is",last_units
